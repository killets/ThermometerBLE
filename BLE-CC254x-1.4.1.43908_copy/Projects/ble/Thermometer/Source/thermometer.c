...
static uint32 runningSeconds=0;  // ÎÞÓÃ
static uint32 remainSeconds=0;  //  µ¹¼ÆÊ±µ½3AM»¹ÓÐ¶àÉÙÃëÆô¶¯record²âÎÂ
static bool timeset_flag = false; // ÊÇ·ñÊÚÊ±£¬ÊÇ²Å¼ì²éÊÇ·ñµ¹¼ÆÊ±Íê³É

#define bodyTempHiAdc 7777  //ÌåÎÂ¶Î¸ß£¬Ð¡ÓÚ²Å¼ÌÐø²â
#define bodyTempLoAdc 1    
#define bodyTempTestCnt 4   // x-1 ´Î²âÊÔ£¬run 3 tests
static uint8 bodyTempInrange = 0; // ÌåÎÂ¶Î²âÊÔÖÐµÄ´ÎÊý 
static uint8 bodyTempTest = bodyTempTestCnt; // run 3 tests


#define bodyTempLogInteval 120000   //Ã¿¸ô¶à³¤Ê±¼ä²âÒ»´Î
        //gj 0804
static uint16 realtime_interval=1;  //ÊµÊ±Ä£Ê½ÏÂ¼ä¸ô
...
static uint8 accelEnabler = FALSE; // ¿ªÆô¼ÓËÙ¶È¼Æ
...
//GJ ACC
static void accelEnablerChangeCB( void ); // µ÷ÓÃ³õÊ¼»¯»òÕßÍ£Ö¹
static void accelRead( void );   //¶ÁÈ¡£¬²¢¸üÐÂ×Ö¶ÎÖµ

static void record_start(void);  //¿ªÊ¼¶¨Ê±Ä£Ê½²âÁ¿ 
...
void Thermometer_Init( uint8 task_id )
{
  ...
  */
  ...
  // Setup the GAP Peripheral Role Profile
  {
    ...
    uint8 initial_advertising_enable = TRUE; //def: FALSE gj  //ÊÇ·ñ¿ª»ú×Ô¶¯¹ã²¥

    // By setting this to zero, the device will go into the waiting state after
    // being discoverable for 30.72 second, and will not being advertising again
    // until the enabler is set back to TRUE
    uint16 gapRole_AdvertOffTime = 0;    //Êµ¼ÊÉÏÊÇ180s,·Ç30s
  ...
  // Setup the Thermometer Characteristic Values
  {
    ...
    thermometerIRange_t thermometerIRange= {0,60}; //²»ÄÜÎª1,·ñÔò Ê±¼ä¼ä¸ô²»ÄÜÉèÎª1£¬>lo
...
//    Thermometer_SetParameter( THERMOMETER_MYTIME, sizeof ( UTCTimeStruct ), &mytime );
  ...
  //gj
  HalAdcInit();
  GAP_SetParamValue(TGAP_LIM_ADV_TIMEOUT, TH_adv_TIMEOUT);  //¿ª»ú¹ã²¥10s
  ...
  //HAL_TURN_ON_LED1();
     ...
     //gj
  osal_start_timerEx( thermometerTaskId, TH_WAKEUP_EVT, TH_wakeup_TIMEOUT); //Ã¿20s»½ÐÑ
...
uint16 Thermometer_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  //gj 
  if (events & TH_WAKEUP_EVT) //´ótimer
  {
...
//   //osal_set_event( thermometerTaskId, TH_START_DISCOVERY_EVT );
    ...
    GAPRole_SetParameter( GAPROLE_ADVERT_ENABLED, sizeof( uint8 ), &adv_enabled );    //ÖØÐÂ¹ã²¥ 
    }
    else
      #if defined(LightOn)
     HAL_TURN_OFF_LED2();
  ...
  {    
    //  HAL_TURN_ON_LED1();
      ...
      {
        if(bodyTempInrange>=2){ //2ÊÇmagic number£¬3´Î²âÊÔ2´ÎÒÔÉÏ²ÅÓÐÐ§
          //start measurement
          #if defined(LightOn)
             HAL_TURN_ON_LED1();
          #endif
  ...
  {   
     if(recorded_cnt < THERMOMETER_MYLOG_LEN/2){ //Ã¿´Î2¸ö×Ö½Ú
...
//gj ACC
  ...
  //periodic indications - if enabled    //indicationÄ£Ê½
...
//         temperature=temp2char(temperature);   
      ...
      double temp = adc2temp(temperature);
      temperature = 0xFF000000 |(uint32)(temp*10);  //²»ÖªÎªÉ¶»ò£¬µ«²»Ó°ÏìÊÖ»úAPPÏÔÊ¾ÎÂ¶È
      #endif
 ...
 * @return  none
 */
static void thermometerCB(uint8 event)  //´¦ÀíÀ´×ÔserviceµÄÊÂ¼þ£¬±ÈÈçÄ³¸öÖµ¸ÄÁË»ò¿ª¹ØÊ¹ÄÜ
{
  switch (event)
  {
 ...
 * @return  none
 */
static void accelRead( void )  //¶ÁXYZ£¬Èç¹û³¬ÏÞ£¬Ôò¸üÐÂ£¬ÇÒµ±XYZ listen notifyÊ±·¢ËÍnotify
{
